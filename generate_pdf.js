...
const ACCENT_BORDER = '#e2e8f0'; // Slate 200

// Helper functions for PDF styling
function drawHeader() {
  // Title Banner Background
  doc.rect(40, 40, 515, 80).fill(DARK_SLATE);
...
doc.fillColor(DARK_SLATE).font('Helvetica-Bold').fontSize(10).text('The Industry Problem', 50, boxY + 10);
...
doc.fillColor(TEXT_MUTED).font('Helvetica').fontSize(8.5).text(
  ...
  '• Mobile-first UI with fixed bottom navigation dock for thumb-reachability on smartphones.\n' +
...
doc.fillColor(DARK_SLATE).font('Helvetica-Bold').fontSize(9.5).text('• Bold Definitive Top Answer Summary:', 40, doc.y);
...
doc.y += 32;
doc.fillColor(DARK_SLATE).font('Helvetica-Bold').fontSize(9.5).text('• Progressive Typewriter Streaming Reveal:', 40, doc.y);
doc.fillColor(TEXT_MUTED).font('Helvetica').fontSize(9).text(
...
doc.y += 32;
doc.fillColor(DARK_SLATE).font('Helvetica-Bold').fontSize(9.5).text('• Mobile View Optimization & Bottom Navigation Dock:', 40, doc.y);
doc.fillColor(TEXT_MUTED).font('Helvetica').fontSize(9).text(
...
doc.y += 32;
doc.fillColor(DARK_SLATE).font('Helvetica-Bold').fontSize(9.5).text('• Touch-Optimized Interactive Safety Calculators:', 40, doc.y);
doc.fillColor(TEXT_MUTED).font('Helvetica').fontSize(9).text(
...
doc.y += 32;
doc.fillColor(DARK_SLATE).font('Helvetica-Bold').fontSize(9.5).text('• Hands-Free Web Speech API Voice Input:', 40, doc.y);
doc.fillColor(TEXT_MUTED).font('Helvetica').fontSize(9).text(
...
const colWidths = [120, 180, 215];

// Table Header
doc.rect(40, tableY, 515, 20).fill(DARK_SLATE);
...
const rows = [
  ...
  ['UI & Styling', 'Tailwind CSS + Glassmorphism', 'High-contrast outdoor theme with touch targets >= 44px.'],
...
rows.forEach((row, i) => {
  ...
  doc.fillColor(DARK_SLATE).font('Helvetica-Bold').fontSize(8.5).text(row[0], 50, rY + 6);
...
doc.fillColor(TEXT_MUTED).font('Helvetica').fontSize(9)
   .text(
     '1. Full-Stack Web Development: Modular React architecture, responsive design, custom hooks, and state management.\n' +
...
doc.fillColor(DARK_SLATE).font('Helvetica-Bold').fontSize(9.5).text('• GitHub Repository:', 50, linksY + 12);
...
doc.fillColor(DARK_SLATE).font('Helvetica-Bold').fontSize(9.5).text('• Live Demo URL:', 50, linksY + 34);
